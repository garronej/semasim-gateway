...
function handle(socket) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    asteriskSocket = asteriskConnections.get(connectionId, imsi);
                    if (!asteriskSocket) {
    ...
    socket.evtResponse.attach(function (sipResponse) {
        var connectionId = misc.cid.read(sipResponse);
        var imsi = misc.readImsi(sipResponse);
        var asteriskSocket = asteriskConnections.get(connectionId, imsi);
        if (!asteriskSocket) {
            return;
        }
