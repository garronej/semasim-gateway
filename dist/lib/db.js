...
_a = f.getUtils(Object.assign({}, c.dbParamsGateway, { "database": "semasim" }), "HANDLE STRING ENCODING"), exports.query = _a.query, exports.esc = _a.esc, exports.buildInsertQuery = _a.buildInsertQuery;
...
function addUaSim(uaSim) {
    return __awaiter(this, void 0, void 0, function* () {
        let sql = "";
        let { imsi, ua } = uaSim;
        sql += exports.buildInsertQuery("ua", {
        ...
        sql += [
            ...
            "FROM ua",
            `WHERE instance= ${exports.esc(ua.instance)} AND user_email= ${exports.esc(ua.userEmail)}`,
            ";",
            ""
        ].join("\n");
        sql += exports.buildInsertQuery("ua_sim", {
            "ua": { "@": "ua_ref" },
            imsi
        }, "IGNORE");
        sql += [
...
function removeUaSim(imsi, uasToKeep = []) {
    return __awaiter(this, void 0, void 0, function* () {
        let condition = uasToKeep.length ? [
            " AND NOT ( ",
            uasToKeep.map(ua => `ua.instance= ${exports.esc(ua.instance)} AND ua.user_email= ${exports.esc(ua.userEmail)}`).join(" OR "),
            " )"
        ].join("") : "";
        yield exports.query([
            "DELETE ua_sim.*",
            ...
            `WHERE ua_sim.imsi= ${exports.esc(imsi)}${condition}`
        ].join("\n"));
    });
}
...
function onSipMessage(toNumber, text, uaSim, date = new Date()) {
    return __awaiter(this, void 0, void 0, function* () {
        let sql = [
            ...
            "WHERE",
            [
                `ua_sim.imsi= ${exports.esc(uaSim.imsi)}`,
                `ua.instance = ${exports.esc(uaSim.ua.instance)}`,
                `ua.user_email= ${exports.esc(uaSim.ua.userEmail)}`
            ].join(" AND "),
            ";",
            ""
        ].join("\n");
        sql += exports.buildInsertQuery("message_toward_gsm", {
            "date": date.getTime(),
            "ua_sim": { "@": "ua_sim_ref" },
            "to_number": toNumber,
            "text": text,
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let row of rows) {
            ...
            let onReceived = () => __awaiter(this, void 0, void 0, function* () {
                return yield exports.query(exports.buildInsertQuery("ua_sim_message_toward_sip", {
                    "id_": row["id_"],
...
function getUnsentMessagesTowardGsm(imsi) {
    return __awaiter(this, void 0, void 0, function* () {
        let rows = yield exports.query([
            ...
            "INNER JOIN ua ON ua.id_ = ua_sim.ua",
            `WHERE ua_sim.imsi=${exports.esc(imsi)} AND message_toward_gsm.send_date IS NULL`,
    ...
    */
    function onSent(messageTowardGsm_id, messageTowardGsm, sendDate) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let sql = exports.buildInsertQuery("message_toward_gsm", {
...
function buildMessageTowardSipInsertQuery(isFromDongle, fromNumber, text, date, bundledData, target) {
    ...
    switch (target.target) {
        case "SPECIFIC UA REGISTERED TO SIM":
            sqlSelectionUaSim += [
                exports.esc(target.uaSim.imsi),
                `ua.instance= ${exports.esc(target.uaSim.ua.instance)}`,
                `ua.user_email= ${exports.esc(target.uaSim.ua.userEmail)}`
            ].join(" AND ");
            break;
        case "ALL UA REGISTERED TO SIM":
            sqlSelectionUaSim += `${exports.esc(target.imsi)}`;
            break;
    ...
    let sql = [
        ...
        "SELECT",
        [
            f.bool.enc(isFromDongle),
            exports.esc(JSON_CUSTOM.stringify(bundledData)),
            exports.esc(date.getTime()),
            exports.esc(fromNumber),
            exports.esc(text)
        ].join(", "),
        sqlSelectionUaSim,
        "HAVING COUNT(*) <> 0",
